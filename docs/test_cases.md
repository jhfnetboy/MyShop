...
- 协议运营方（治理者）：G-01（读配置）、G-02/G-03/G-04（改费率/金库/上架费）、G-05（动作白名单）
  - 期望摘要：读写成功；deny 后购买 revert，allow 后购买成功；暂停后购买失败
- 店铺运营者（Shop Owner/Operator）：S-01（注册）、S-04（授权）、I-01（上架普通）、I-02（上架串号）、I-04（下架）
  - 期望摘要：计数正确；串号商品无 permit 购买失败；下架后购买失败
- 买家（Buyer）：B-01（ERC20 购买）、B-02（串号购买）、B-03（过期）、B-04（重放）、B-05（参数不匹配）、B-06（店铺暂停）、B-07（动作被 deny）
  - 期望摘要：成功/失败行为与日志符合预期；Worker 查询能看到 enrich 的记录
- 运维/社区节点（IPFS/选配）：部署 Kubo/Cluster/Nginx/Ingress，Pin 文档 CID，前端配置 `IPFS_GATEWAY`，点击“测试网关”，验证类别文档链接
  - 期望摘要：主/备网关可达；CID 能打开；不使用 IPFS 时流程照常
...
- 期望：被授权地址能执行对应动作（上架、维护、配置 action 等）。
 - 命令模板（示意）：通过页面完成授权；或参考 cast send 发送对应授权交易（函数签名以合约为准）

### S-05 暂停店铺

- 页面入口：`#/shop-console`（或 `#/protocol-console` 以治理者身份）
- 期望：暂停后购买失败（见 B-06）。
 - 命令模板（示意）：通过页面完成暂停；或参考 cast send 发送暂停交易（函数签名以合约为准）
